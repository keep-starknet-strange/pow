...
export function useWalletConnect() {
  ...
  const initializeProvider = async () => {
    ...
    const metadata = {
      name: "POW Game",
      description: "Starknet POW Game",
      url: "https://pow.co/",
      icons: ["https://avatars.githubusercontent.com/u/118723009"],
    };
  ...
  const openUriForWallet = async (
    ...
    // const candidates =
      wallet === "argent"
        ...
        : 
            ...
            // `https://braavos.co/wc?uri=${encodedUri}`,
    ...
    //   );
    //   if (canUniversal) {
    //     console.log("opening universal url", universal);
  ...
  const connectWallet = async (wallet: "ready" | "braavos") => {
    if (!provider) return;
    ...
    try {
      ...
      for (const chain of chainCandidates) {
        ...
        if (uri) {
          const encodedUri = encodeURIComponent(uri);
          await Linking.openURL(`${wallet}://wc?uri=${encodedUri}`);
        }
  ...
  const connectReady = async () => connectWallet("ready");
