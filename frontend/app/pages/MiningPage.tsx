...
export const MiningPage: React.FC<MiningPageProps> = (props) => {
  ...
  const tryMineBlock = () => {
    ...
    notify("TryMineBlock", {
      ...
      isMined: newMineCounter >= currentBlock.hp,
    });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (mineCounter < currentBlock.hp) {
        tryMineBlock();
      } else {
  ...
  }, [upgradableGameState.minerSpeed, mineCounter, currentBlock.hp]);
  ...
  return (
    <View className="flex-1 flex flex-col items-center mt-[30%]">
      ...
      <Text className="text-[#f7f7f7] text-2xl mt-4">Difficulty {currentBlock.hp}</Text>
      <Text className="text-[#f7f7f7] text-2xl">Nonce {nonce}</Text>
      <Text className="text-[#f7f7f7] text-2xl">Hash {blockHash}</Text>
      <Text className="text-[#f7f7f7] text-2xl mt-8">Reward ₿ {currentBlock.reward}</Text>
      <Text className="text-[#f7f7f7] text-2xl">Fees ₿ {currentBlock.fees.toFixed(2)}</Text>
    </View>
  );
};
