  ...
  useEffect(() => {
    ...
    const getLeaderboard = async () => {
      ...
      const users = scores.map((score: any, index: number) => {
        const shortAddress = score.user.slice(0, 6) + "..." + score.user.slice(-4);
        return {
          address: score.user,
          balance: score.new_balance,
          prestige: 0, // TODO
          nouns: getRandomNounsAttributes(), // TODO
          name: accounts?.accounts.find((account: any) => account.address === score.user)?.username || shortAddress,
          id: index + 1,
        }
      });
  ...
  return (
    ...
    >
     <View className="flex flex-row justify-end items-center p-2">
       <Text className="text-[#e7e7e7] text-4xl font-bold mr-2 font-Xerxes">🏆 Rankings</Text>
     </View>
     <View className="flex flex-row justify-between items-center p-4 transparent">
       <Text className="text-lg font-bold text-white flex-1 font-Pixels">Leaderboard</Text>
       <Text className="text-lg font-bold text-white w-[5rem] text-center font-Pixels">Prestige</Text>
       <Text className="text-lg font-bold text-white w-[6rem] text-right font-Pixels">Score</Text>
     </View>
     <ScrollView className="flex-1">
       {leaderboard.map((user, index) => (
         <View key={user.id} className={`flex flex-row justify-between items-center p-2 ${index % 2 === 0 ? 'bg-[#10111910]' : 'bg-[#10111920]'}`}>
           <View className="flex flex-row items-center flex-1">
             <View className="w-[4rem] aspect-square mr-2 rounded-xl overflow-hidden bg-[#11111160]
                              border-2 border-[#e7e7e740] shadow-lg shadow-black/20
             ">
               <PFPView user={user.address} attributes={user.nouns} />
             </View>
             <Text className="text-2xl font-bold text-white font-Pixels">{user.name}</Text>
           </View>
           ...
           <Text className="text-xl text-white w-[6rem] text-right font-bold font-Pixels">{shortMoneyString(user.balance)}</Text>
         </View>
       ))}
     </ScrollView>
   </ImageBackground>
