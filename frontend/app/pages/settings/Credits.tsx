...
export const CreditsSection = () => {
  ...
  return (
    <View className="">
      ...
      <View className="mt-2">
        <Text className="text-white text-lg font-bold">This app was created by StarkWare's Exploration Apps team.</Text>
        {creditedPeople.map((person, index) => (
          <Text key={index} className="text-white text-md w-full text-center mt-2">
            {person.name} - {person.role}
          </Text>
        ))}
        <Text className="text-white text-lg font-bold mt-4">Special thanks to:</Text>
