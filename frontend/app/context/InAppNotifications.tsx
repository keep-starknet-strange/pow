...
const InAppNotificationsContext = React.createContext<InAppNotificationsContextType | undefined>(undefined);

export const useInAppNotifications = () => {
  const context = useContext(InAppNotificationsContext);
  if (!context) {
    throw new Error("useInAppNotifications must be used within an InAppNotificationsProvider");
  }
  return context;
}

export const InAppNotificationsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [inAppNotifications, setInAppNotifications] = useState<InAppNotificationType[]>([]);

  const notificationLimit = 5;
  const sendInAppNotification = useCallback((notificationTypeId: number, message?: string) => {
    // Add a new notification to the state ( and remove the oldest one if the limit is reached )
    setInAppNotifications(prev => {
      const notifMsg = message || inAppNotificationsJson[notificationTypeId].message || "Unknown notification type";
  ...
  return (
    <InAppNotificationsContext.Provider value={{ inAppNotifications, sendInAppNotification }}>
      {children}
