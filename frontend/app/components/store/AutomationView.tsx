...
export const AutomationView: React.FC<AutomationViewProps> = (props) => {
  ...
  const [currAutomationIcon, setCurrAutomationIcon] = useState(getAutomationIcons(props.chainId)[props.automation.name][props.automation.levels[0].name]);
  const [automationIcons, setAutomationIcons] = useState(getAutomationIcons(props.chainId)[props.automation.name]);
  useEffect(() => {
    const automationLevel = (automations[props.chainId][props.automation.id] + 1) || 0;
    setLevel(automationLevel);
    const automationIcons = getAutomationIcons(props.chainId)[props.automation.name];
    setCurrAutomationIcon(automationLevel === 0 ? automationIcons["None"] : automationIcons[props.automation.levels[automationLevel - 1].name]);
  ...
  return (
    <View className="flex flex-row w-full items-center">
      ...
      >
        <Image source={currAutomationIcon} className="w-[5.4rem] h-[5.4rem] rounded-lg" />
      </View>
      <View className="flex flex-col justify-start items-start ml-2 gap-1 flex-1">
        <View className="flex flex-col justify-start items-start ml-2 gap-1 flex-1">
          <Text className="text-[#e7e7e7] text-xl font-bold">{props.automation.name}</Text>
          <Text className="text-[#e7e7e7] text-md mb-[1rem]">{props.automation.description}</Text>
        </View>
        <View className="flex flex-row justify-start items-start ml-2 gap-1 flex-1">
          {props.automation.levels.map((automation: any, index: number) => (
            ...
            >
              <Image source={automationIcons[automation.name]} className="w-[2rem] h-[2rem]" />
               {level < index && (
