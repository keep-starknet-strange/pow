  ...
  return (
    <View className="w-full h-full flex flex-col items-center justify-center">
      <View className="flex-1 bg-[#ffff8008] aspect-square rounded-xl border-2 border-[#ffff80b0] relative overflow-hidden">
              ...
              index !== (props.block?.transactions.length || 1) - 1 && (
                <View
                  ...
                  style={{
                    width: `${txWidth}%`,
                    height: `${txWidth}%`,
                    ...getTxStyle(props.chainId, tx.typeId),
                  }}
                >
                  <Image
        ...
        {props.completed && (
          <View className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-between bg-[#171717c0] rounded-xl">
            ...
            <View className="flex flex-col items-end justify-center w-full px-2">
              <View className="flex flex-row items-center gap-1">
                <Image source={feeImg} className="w-4 h-4" />
                <Text className="text-[#e9e980f0] text-lg font-bold">
                  {shortMoneyString(
                    (props.block?.fees || 0) + (props.block?.reward || 0),
                  )}
                </Text>
              </View>
            </View>
