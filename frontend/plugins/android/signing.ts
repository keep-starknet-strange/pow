...
const withSigningConfig: ConfigPlugin = (config) => {
    return withAppBuildGradle(config, (config) => {
        ...
        if (config.modResults.language === "kt") {
            throw new Error("Current plugin can only modify groovy build.gradle files");
        }

        console.log("📝 Modifying android/app/build.gradle to include release config");
        let gradle = config.modResults.contents;
        let withSigningConfigs = attachReleaseSigningConfig(gradle);
        config.modResults.contents = setupReleaseSigningConfigInBuildTypes(withSigningConfigs);
...
function setupReleaseSigningConfigInBuildTypes(content: string): string {
    const signingConfig = new RegExp("(buildTypes {(\n|.)*?release {(\n|.)*?signingConfigs\.)(debug)");
...
const withKeystoreProperties: ConfigPlugin = (config) => {
    return withGradleProperties(config, (config) => {
        ...
        if (fileName == null || password == null || keyAlias == null || keyPassword == null) {
            console.warn("⚠️  Could not find appropriate env variables to set the keystore details. Ignoring...");
            return config;
        }
