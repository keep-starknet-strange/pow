...
for entry in $(echo $L1_TRANSACTIONS | jq -r '. | @base64'); do
    ...
    starkli invoke --network sepolia --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $POW_CONTRACT_ADDRESS setup_transaction_config $SETUP_TRANSACTION_CALLDATA
done
...
starkli invoke --network sepolia --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $POW_CONTRACT_ADDRESS set_next_chain_cost $NEXT_CHAIN_COST

# Set dapps unlock cost
DAPPS_UNLOCK_COST=100000000
echo "Setting dapps unlock cost to $DAPPS_UNLOCK_COST"
starkli invoke --network sepolia --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $POW_CONTRACT_ADDRESS set_dapps_unlock_cost $DAPPS_UNLOCK_COST
