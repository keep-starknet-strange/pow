...
for entry in $(echo $L1_UPGRADES | jq -r '. | @base64'); do
    ...
    starkli invoke --network sepolia --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $POW_GAME_CONTRACT_ADDRESS setup_upgrade_config $SETUP_UPGRADE_CALLDATA
done
...
for entry in $(echo $L1_AUTOMATIONS | jq -r '. | @base64'); do
    ...
    starkli invoke --network sepolia --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $POW_GAME_CONTRACT_ADDRESS setup_automation_config $SETUP_AUTOMATION_CALLDATA
done
...
for entry in $(echo $L1_TRANSACTIONS | jq -r '. | @base64'); do
    ...
    starkli invoke --network sepolia --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $POW_GAME_CONTRACT_ADDRESS setup_transaction_config $SETUP_TRANSACTION_CALLDATA
done
...
starkli invoke --network sepolia --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $POW_GAME_CONTRACT_ADDRESS setup_prestige_config $SETUP_PRESTIGE_CALLDATA
echo
...
starkli invoke --network sepolia --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $POW_GAME_CONTRACT_ADDRESS set_next_chain_cost $NEXT_CHAIN_COST
...
starkli invoke --network sepolia --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $POW_GAME_CONTRACT_ADDRESS set_dapps_unlock_cost 0 $L1_DAPPS_UNLOCK_COST
echo "Setting L2 dapps unlock cost to $L2_DAPPS_UNLOCK_COST (from unlocks config)"
starkli invoke --network sepolia --keystore $STARKNET_KEYSTORE --account $STARKNET_ACCOUNT --watch $POW_GAME_CONTRACT_ADDRESS set_dapps_unlock_cost 1 $L2_DAPPS_UNLOCK_COST
