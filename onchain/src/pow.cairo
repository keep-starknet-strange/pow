    ...
    impl PowGameActionsImpl of IPowGameActions<ContractState> {
        ...
        fn execute_actions(ref self: ContractState, actions: Span<PowActions>) {
            ...
            loop {
                ...
                } else if action_name == 'add_transaction_bundled' {
                    let chain_id = *calldata.at(0);
                    // Extract tx_type_ids from calldata (skip first element which is chain_id)
                    let mut tx_type_ids: Array<u32> = ArrayTrait::new();
                    let mut j: u32 = 1;
                    loop {
                        if j >= calldata.len() {
                            break;
                        }
                        tx_type_ids.append(*calldata.at(j));
                        j += 1;
                    };
                    self.add_transaction_bundled(chain_id, tx_type_ids.span());
                } else if action_name == 'mine_block' {
                    let chain_id = *calldata.at(0);
                    self.mine_block(chain_id);
                } else if action_name == 'mine_block_bundled' {
                    let chain_id = *calldata.at(0);
                    let clicks = *calldata.at(1);
                    self.mine_block_bundled(chain_id, clicks);
