...
pub mod PrestigeComponent {
  ...
  use pow_game::prestige::interface::{
    IPrestige, PrestigeConfig, PrestigeSetupParams
  };
  ...
  > of IPrestige<ComponentState<TContractState>> {
    // Game config
    fn get_prestige_config(self: @ComponentState<TContractState>, prestige_id: u32) -> PrestigeConfig {
    ...
    fn setup_prestige(ref self: ComponentState<TContractState>, params: PrestigeSetupParams) {
      ...
      while idx != max_prestige {
        let prestige = PrestigeConfig {
          cost: *params.costs[idx],
          scaler: *params.scalers[idx],
        };
        self.prestiges.write(idx, prestige);
        idx += 1;
      }
      self.emit(
        PrestigeConfigUpdated {
          new_config: params
        }
      );
    }
